...
class PersonController: ControllerBase {
    override func handleCommand(command: Command, data: [String]) -> View? {
        ...
        default:                return super.handleCommand(command, data: data)
        }
    }
...
class CityController: ControllerBase {
    override func handleCommand(command: Command, data: [String]) -> View? {
        switch command {
        case .LIST_CITIES:      return listAll()
        case .SEARCH_CITIES:    return search(data[0])
        case .DELETE_CITY:      return delete(data[0])
        default:                return super.handleCommand(command, data: data)
        }
    }
    ...
    private func delete(city: String) -> View {
        ...
        repository.People
            ...
            .forEach { repository.removePerson($0.name) }
        return PersonListView(data: repository.People)
    }
}